 ...
 */
 ...
 {
   /* On d�truit chaque objet contenu dans la liste */
   int i = 0;

   if(obj2s_existe(*liste)) {
     for(i = 0; i < (*(*liste)).nb; i++) {
       obj2_detruire(&((*liste)->liste[i])); /* On vide la liste */
       (*liste)->liste[i] = NULL;
     }

     free((*liste)->liste);
     free(*liste);
     *liste = NULL; /* Et enfin on lib�re la liste, permettant d'�tre sur de ne jamais tenter de la free encore une fois */
